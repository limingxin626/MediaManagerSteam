  ...
  <div
    ...
    <div class="px-4 pt-3">
      ...
      <div v-if="messageTags.length > 0 || message.media_count > 0" class="flex items-center gap-2 mt-2 flex-wrap">
        ...
        <span v-if="message.media_count > 0" class="inline-flex items-center gap-1 text-xs text-[var(--text-muted)] ml-auto tabular-nums">
      ...
      <div class="flex items-center justify-between mt-3 pt-2 pb-3 border-t border-[var(--divider)] text-xs text-[var(--text-muted)]">
          ...
          <div class="flex items-center gap-1">
            ...
            <button @click.stop="handleToggleStar" class="p-1.5 rounded-[var(--radius-sm)] transition-colors" :class="props.message.starred
              ? 'text-amber-400 hover:text-amber-500'
              : 'text-[var(--text-muted)] hover:text-amber-400 hover:bg-[var(--bg-secondary)]'"
            ...
            <button @click.stop="handleEdit"
              class="p-1.5 text-[var(--text-muted)] hover:text-[var(--color-primary-600)] hover:bg-[var(--bg-secondary)] rounded-[var(--radius-sm)] transition-colors"
              title="编辑消息">
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                  d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            ...
            <button @click.stop="handleDelete"
              class="p-1.5 text-[var(--text-muted)] hover:text-[var(--color-danger)] hover:bg-[var(--bg-secondary)] rounded-[var(--radius-sm)] transition-colors"
              title="删除消息">
              <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div class="shrink-0 flex flex-col items-center gap-2 mb-1">
      ...
      <div v-if="props.selectable" @click.stop="emit('toggle-select', props.message.id)"
        class="w-6 h-6 rounded-md border-2 flex items-center justify-center transition-colors cursor-pointer" :class="props.selected
          ...
          : 'border-[var(--border-strong)] hover:border-[var(--color-primary-500)]'">
      ...
      <button @click="handleClick"
        class="w-8 h-8 rounded-full bg-[var(--bg-card)] border border-[var(--border-color)] text-[var(--text-secondary)] hover:text-[var(--color-primary-600)] hover:border-[var(--color-primary-500)] flex items-center justify-center shadow-[var(--shadow-sm)] transition-colors active:scale-95"
        title="查看详情">
