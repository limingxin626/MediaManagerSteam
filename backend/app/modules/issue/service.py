...
from app.modules.issue.schemas import IssueStatus
...
def get(db: Session, issue_id: int) -> Issue:
    issue = db.get(Issue, issue_id)
    if issue is None: raise LookupError("issue not found")
    return issue
...
def create_use_case(db: Session, title: str, description: str | None, status: IssueStatus) -> Issue:
    issue = create_issue(db, title, description, status)
    commit(db)
    db.refresh(issue)
    return issue


def update_use_case(db: Session, issue_id: int, title: str | None, description: str | None) -> Issue:
    issue = get(db, issue_id)
    update_issue(db, issue, title, description)
    commit(db)
    db.refresh(issue)
    return issue


def move_use_case(db: Session, issue_id: int, status: IssueStatus, position: int) -> Issue:
    issue = get(db, issue_id)
    move_issue(db, issue, status, position)
    commit(db)
    db.refresh(issue)
    return issue


def delete_use_case(db: Session, issue_id: int) -> None:
    delete_issue(db, get(db, issue_id))
    commit(db)
