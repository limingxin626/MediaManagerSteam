...
def cleanup_orphan_tags(db: Session) -> int:
    """删除没有任何 message 关联的 tag，由外层用例提交。"""
    orphans = (
        db.query(Tag)
...
def reorder_message_media(db: Session, message_id: int, media_ids: List[int]) -> bool:
    """按传入的 media_id 顺序重置 position，由外层用例提交。"""
...
def create_message_service(
    ...
    for idx, file_path in enumerate(files):
        ...
        result = process_file(db, file_path, media_id=media_id_arg)
...
def update_message_service(
    ...
    if media_order is not None:
        ...
        if not reorder_message_media(db, message_id, media_order):
