...
from app.modules.smart import clip_service
from app.shared.unit_of_work import commit

logger = logging.getLogger(__name__)


def fetch_media_in_order(db: Session, scored: List[dict]) -> List[Media]:
    if not scored: return []
    rows = db.query(Media).filter(Media.id.in_([item["media_id"] for item in scored])).all()
    by_id = {media.id: media for media in rows}
    return [by_id[item["media_id"]] for item in scored if item["media_id"] in by_id]
...
def find_similar(db: Session, media_id: int, limit: int = 50) -> List[dict]:
    ...
    result = [{"media_id": ids[i], "score": float(scores[i])} for i in order]
    commit(db)
    return result
