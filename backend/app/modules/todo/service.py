...
from app.modules.todo.schemas import TodoStatus
...
def get(db: Session, todo_id: int) -> Todo:
    ...
    if todo is None: raise LookupError("todo not found")
    return todo


def create_use_case(db: Session, title: str, status: TodoStatus) -> Todo:
    todo = create_todo(db, title, status); commit(db); db.refresh(todo); return todo


def update_use_case(db: Session, todo_id: int, title: str) -> Todo:
    todo = get(db, todo_id); update_title(db, todo, title); commit(db); db.refresh(todo); return todo


def move_use_case(db: Session, todo_id: int, status: TodoStatus, position: int) -> Todo:
    todo = get(db, todo_id); move_todo(db, todo, status, position); commit(db); db.refresh(todo); return todo


def delete_use_case(db: Session, todo_id: int) -> None:
    delete_todo(db, get(db, todo_id)); commit(db)
