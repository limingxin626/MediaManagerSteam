...
def rescan(repo_id: Optional[str] = None, session_factory=None) -> Optional[dict]:
    ...
    try:
        ...
        for rid, root in selected.items():
            result = _scan_repository(rid, root, roots - {os.path.normcase(os.path.abspath(root))}, session_factory)
            if result is None:
...
def _scan_repository(repo_id: str, root: str, other_roots: set[str], session_factory=None) -> Optional[dict]:
    ...
    try:
        ...
        db.flush()
        from app.modules.repository.folder_service import ensure_folders
