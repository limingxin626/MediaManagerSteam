...
def start_worker(settings: AppConfig | None = None, session_factory=None) -> None:
    global _thread
    with _guard:
        if _thread is not None and _thread.is_alive():
            return
        _stop.clear()
        _thread = threading.Thread(target=_run_loop, args=(settings or get_settings(), session_factory), name="catalog-materializer", daemon=True)
        _thread.start()
...
def _run_loop(settings: AppConfig, session_factory=None) -> None:
    with use_settings(settings):
        while not _stop.is_set():
            try:
                progress = _process_batch(session_factory=session_factory)
            except Exception:
...
def _process_batch(limit: int = 20, session_factory=None) -> int:
...
def _process_row(db, row: RepositoryFile) -> bool:
    ...
    try:
        ...
        result = process_file(db, absolute)
        if result is None:
