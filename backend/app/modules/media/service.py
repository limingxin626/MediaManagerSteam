...
def create_preview_media(db: Session, file_path: str) -> Media:
    """为视频预览图强制创建一条新的 Media 行(不去重)。"""
    result = process_file(db, file_path, skip_dedup=True)
    if result is None:
        raise ValueError("Failed to process preview file")
    return result["media"]


def update_preview(db: Session, preview_id: int, frame_ms: int | None, start_ms: int | None, end_ms: int | None) -> Media:
    image = db.get(Media, preview_id)
    if image is None or image.video_media_id is None:
        raise LookupError("Preview not found")
    new_frame = frame_ms if frame_ms is not None else image.frame_ms
    new_start = start_ms if start_ms is not None else image.start_ms
    new_end = end_ms if end_ms is not None else image.end_ms
    _validate_preview_range(new_frame, new_start, new_end)
    if frame_ms is not None: image.frame_ms = frame_ms
    if start_ms is not None: image.start_ms = start_ms
    if end_ms is not None: image.end_ms = end_ms
    commit(db); db.refresh(image)
    return image


def delete_preview(db: Session, preview_id: int) -> None:
    image = db.get(Media, preview_id)
    if image is None or image.video_media_id is None:
        raise LookupError("Preview not found")
    file_path = config.resolve_to_absolute(image.repo_id, image.file_path)
    db.delete(image); commit(db)
    for path in (config.get_thumbnail_path(preview_id), file_path):
        if path and os.path.exists(path):
            try: os.remove(path)
            except OSError: logger.warning("Failed to remove preview file %s", path)


def set_starred(db: Session, media_id: int, starred: bool) -> None:
    media = db.get(Media, media_id)
    if media is None: raise LookupError("Media not found")
    media.starred = 1 if starred else 0; commit(db)


def set_rating(db: Session, media_id: int, rating: int) -> None:
    media = db.get(Media, media_id)
    if media is None: raise LookupError("Media not found")
    media.rating = rating; commit(db)


def set_tags(db: Session, media_id: int, tag_ids: list[int]) -> list:
    from app.models import Tag
    media = db.get(Media, media_id)
    if media is None: raise LookupError("Media not found")
    media.tags = db.query(Tag).filter(Tag.id.in_(tag_ids)).all(); commit(db)
    return media.tags


def set_people(db: Session, media_id: int, person_ids: list[int]) -> list:
    from app.models import Person
    media = db.get(Media, media_id)
    if media is None: raise LookupError("Media not found")
    media.people = db.query(Person).filter(Person.id.in_(person_ids)).all(); commit(db)
    return media.people
...
def attach_screenshot_preview(
    ...
    try:
        ...
        image = create_preview_media(db, tmp_path)
