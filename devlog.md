...
1. **文件缺失**：新增 `/admin/missing-files`，默认勾选已加载的缺失媒体，可通过全局按钮批量删除逻辑 Media；部分失败时保留失败项以便重试。
2. **重复文件**：默认勾选非当前主路径副本，使用固定在页面底部的全局“删除所选”跨组清理。
3. **管理布局**：数据库管理标题和标签栏固定，子页面使用独立纵向滚动区。
4. **后端过滤**：`GET /media` 支持 `has_physical_file`，用于查询有无已物化副本的媒体。

验证：Vue `pnpm build` 通过；相关后端测试此前通过；未对真实媒体执行永久删除。
